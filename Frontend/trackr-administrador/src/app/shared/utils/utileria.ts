...
export function obtenerUrlComprasEnLineaFrontend() {
  ...
  return baseUrl + '/compras_qa/#/';
}

export function obtenerNombre(usuario: Usuario): string {
  return (
    usuario.nombre +
    ' ' +
    (usuario.apellidoPaterno === null ? '' : usuario.apellidoPaterno) +
    ' ' +
    (usuario.apellidoMaterno === null ? '' : usuario.apellidoMaterno)
  );
}
...
export function validarFormatoCorreo(correo: string): boolean {
  let valido = false;
  ...
  if (correo === undefined || correo === null || correo === '') {
    ...
    valido = correo_regex.test(correo)
  }
